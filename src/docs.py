...
def cache_key(query, version=Config.DEFAULT_CAKE_VERSION):
...
def search(query=None, version=Config.DEFAULT_CAKE_VERSION, limit=Config.RESULT_COUNT):
    if query:
...
def main(wf):
    ...
    if not query:
        wf.add_item("Search the CakePHP docs...")
        wf.send_feedback()
        return 0
    ...
    query = []
    version = Config.DEFAULT_CAKE_VERSION

    for word in words:
        if word in Config.SUPPORTED_CAKE_VERSIONS:
    ...
    if not results:
        url = "https://www.google.com/search?q={}".format(
            quote_plus('"CakePHP" {}'.format(query))
        )
        wf.add_item(
...
if __name__ == "__main__":
    wf = Workflow3(
        update_settings={"github_slug": "techouse/alfred-cakephp-docs", "frequency": 7}
    )
    log = wf.logger
